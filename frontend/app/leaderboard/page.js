...
export default function LeaderboardPage() {
  ...
  useEffect(() => {
    ...
    socket.on("connect", () => {
      console.log("Connected to WebSocket server from LeaderboardPage");
    });
  ...
  return (
    <div className="pt-header min-h-screen bg-dark text-light p-4 md:p-6">
      ...
      <div className="flex items-center justify-between mb-8 relative">
        <h1 className="text-2xl md:text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-primary to-primary-light">
          Leaderboard
        </h1>
      ...
      <div className="max-w-4xl mx-auto">
        {leaderboard.length === 0 ? (
          <div className="py-16 flex flex-col items-center justify-center text-center bg-dark-lighter border border-dashed border-dark-lightest rounded-2xl">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              className="h-16 w-16 text-light-darkest mb-4"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
            ...
            <p className="text-light-darkest max-w-md">
              No teams have been completed yet. Be the first to complete your team and claim the
                    ...
                    <div className="flex items-center">
                      <span
                        className={`font-medium ${
                          user && entry.userId === user.id ? "text-primary" : "text-light"
                        }`}
                      >
                        {entry.username}
                      </span>
        ...
        {leaderboard.length > 0 && (
          <div className="mt-6 flex flex-col md:flex-row justify-between items-center text-sm text-light-darkest">
            ...
            {user && leaderboard.findIndex((entry) => entry.userId === user.id) !== -1 ? (
              <p>
                Your Rank: #{leaderboard.findIndex((entry) => entry.userId === user.id) + 1} of{" "}
                {leaderboard.length}
              </p>
            ) : (
